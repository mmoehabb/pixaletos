...
function App() {
  const { addLayer, layers, dimensions, zoom, currentTool, setZoom } = useAppStore();
  ...
  return (
    <div className="flex flex-col h-screen w-screen bg-neutral-950 overflow-hidden text-neutral-200 font-sans">
      ...
      <div className="h-10 bg-neutral-900 border-b border-neutral-800 flex items-center px-4 flex-shrink-0 z-20">
        <div className="font-semibold text-indigo-400 tracking-wide text-sm mr-8">PIXEL AI</div>
      ...
      <div className="h-6 bg-indigo-950 border-t border-indigo-900 flex items-center px-3 justify-between text-xs text-indigo-300 flex-shrink-0 z-20">
        <div className="flex gap-4">
           <span>Tool: <span className="text-white capitalize">{currentTool}</span></span>
           <span>Size: {dimensions.width}x{dimensions.height}</span>
        </div>
        <div className="flex items-center gap-2">
           <button onClick={() => setZoom(Math.max(0.1, zoom - 0.5))} className="hover:text-white">-</button>
           <span className="w-12 text-center text-white">{Math.round(zoom * 100)}%</span>
           <button onClick={() => setZoom(Math.min(20, zoom + 0.5))} className="hover:text-white">+</button>
        </div>
      </div>
    </div>
  );
