...
import { Container, Sprite, Graphics as PixiGraphics, Texture, BufferImageSource } from "pixi.js";
import { useAppStore } from "../../store";
import type { Command } from "../../types";

extend({ Container, Sprite, Graphics: PixiGraphics });

const CanvasLayer = ({ data, opacity, visible, dimensions, zIndex }: { data: Uint8ClampedArray, opacity: number, visible: boolean, dimensions: {width:number, height:number}, zIndex?: number }) => {
...
const SelectionOverlay = ({ dimensions, selection }: { dimensions: {width:number, height:number}, selection: Uint8Array | null }) => {
...
export const PixelCanvas: React.FC = () => {
  const {
    dimensions, layers, zoom, pan, currentTool, setPan, activeLayerId,
    foregroundColor, executeCommand, updateLayerData, selection, setSelection, setForegroundColor
  } = useAppStore();
  ...
  // We need previewData as state to trigger re-renders for the CanvasLayer
  const [previewDataVersion, setPreviewDataVersion] = useState(0);
  const [previewData, setPreviewData] = useState<Uint8ClampedArray>(new Uint8ClampedArray(dimensions.width * dimensions.height * 4));

  // Update previewData array size when dimensions change
  useEffect(() => {
    setPreviewData(new Uint8ClampedArray(dimensions.width * dimensions.height * 4));
  }, [dimensions]);

  // We will directly mutate layer.data in pencil/eraser to be fast, but we will keep track of diffs
  const strokeDiff = useRef<Map<number, {oldColor: {r:number,g:number,b:number,a:number}, newColor: {r:number,g:number,b:number,a:number}}>>(new Map());
  ...
  const getPixel = (data: Uint8ClampedArray, x: number, y: number) => {
     if (x < 0 || x >= dimensions.width || y < 0 || y >= dimensions.height) return {r:0,g:0,b:0,a:0};
     const i = (y * dimensions.width + x) * 4;
     return { r: data[i], g: data[i+1], b: data[i+2], a: data[i+3] };
  };

  const drawPixel = (data: Uint8ClampedArray, x: number, y: number, color: {r: number, g: number, b: number, a: number}, trackDiff: boolean = true) => {
    if (x < 0 || x >= dimensions.width || y < 0 || y >= dimensions.height) return;
    if (selection && !selection[y * dimensions.width + x]) return;

    const i = (y * dimensions.width + x) * 4;
    const oldR = data[i], oldG = data[i+1], oldB = data[i+2], oldA = data[i+3];

    if (oldR === color.r && oldG === color.g && oldB === color.b && oldA === color.a) return;
  ...
  const drawLine = (data: Uint8ClampedArray, x0: number, y0: number, x1: number, y1: number, color: {r: number, g: number, b: number, a: number}, trackDiff: boolean = true) => {
  ...
  const drawRect = (data: Uint8ClampedArray, x0: number, y0: number, x1: number, y1: number, color: {r: number, g: number, b: number, a: number}, trackDiff: boolean = true) => {
  ...
  const drawEllipse = (data: Uint8ClampedArray, x0: number, y0: number, x1: number, y1: number, color: {r: number, g: number, b: number, a: number}, trackDiff: boolean = true) => {
  ...
  const floodFill = (data: Uint8ClampedArray, startX: number, startY: number, color: {r: number, g: number, b: number, a: number}, trackDiff: boolean = true) => {
    const targetColor = getPixel(data, startX, startY);
    if (targetColor.r === color.r && targetColor.g === color.g && targetColor.b === color.b && targetColor.a === color.a) return;

    const match = (x: number, y: number) => {
       if (selection && !selection[y * dimensions.width + x]) return false;
       const p = getPixel(data, x, y);
       return p.r === targetColor.r && p.g === targetColor.g && p.b === targetColor.b && p.a === targetColor.a;
    };

    const stack = [[startX, startY]];
    while(stack.length) {
       const [x, y] = stack.pop()!;
       if (x >= 0 && x < dimensions.width && y >= 0 && y < dimensions.height && match(x, y)) {
  ...
  const commitDrawing = (layerId: string, diffEntries: [number, {oldColor: any, newColor: any}][]) => {
  ...
  const applyPreviewToLayer = (layerId: string) => {
    ...
    if (startDrawPos.current && lastDrawPos.current) {
        if (currentTool === 'line') {
            drawLine(layer.data, startDrawPos.current.x, startDrawPos.current.y, lastDrawPos.current.x, lastDrawPos.current.y, color, true);
        } else if (currentTool === 'rectangle') {
            drawRect(layer.data, startDrawPos.current.x, startDrawPos.current.y, lastDrawPos.current.x, lastDrawPos.current.y, color, true);
        } else if (currentTool === 'ellipse') {
            drawEllipse(layer.data, startDrawPos.current.x, startDrawPos.current.y, lastDrawPos.current.x, lastDrawPos.current.y, color, true);
        }
    }
  ...
  const handlePointerDown = (e: React.PointerEvent<HTMLDivElement>) => {
    ...
    if (currentTool === 'eyedropper') {
        const topVisibleLayer = [...layers].find(l => l.visible && getPixel(l.data, x, y).a > 0);
        if (topVisibleLayer) {
    ...
    if (currentTool === 'move' && selection) {
        ...
        movedSelectionData.current = new Uint8Array(dimensions.width * dimensions.height);
            ...
            if (selection[i]) {
                ...
                if (!strokeDiff.current.has(pxIdx)) {
                    strokeDiff.current.set(pxIdx, {
                        oldColor: {r: layer.data[pxIdx], g: layer.data[pxIdx+1], b: layer.data[pxIdx+2], a: layer.data[pxIdx+3]},
    ...
    const color = currentTool === 'eraser' ? { r: 0, g: 0, b: 0, a: 0 } : hexToRgb(foregroundColor);
  ...
  const handlePointerMove = (e: React.PointerEvent<HTMLDivElement>) => {
    ...
    if (currentTool === 'move' && selection && originalLayerData.current && startDrawPos.current) {
        lastDrawPos.current = { x, y };
        ...
        for (let py = 0; py < dimensions.height; py++) {
            for (let px = 0; px < dimensions.width; px++) {
                if (movedSelectionData.current && movedSelectionData.current[py * dimensions.width + px]) {
                    const srcIdx = (py * dimensions.width + px) * 4;
                    const destX = px + dx;
                    const destY = py + dy;
                    if (destX >= 0 && destX < dimensions.width && destY >= 0 && destY < dimensions.height) {
                        const destIdx = (destY * dimensions.width + destX) * 4;
                        previewData[destIdx] = originalLayerData.current[srcIdx];
                        previewData[destIdx+1] = originalLayerData.current[srcIdx+1];
                        previewData[destIdx+2] = originalLayerData.current[srcIdx+2];
                        previewData[destIdx+3] = originalLayerData.current[srcIdx+3];
                    }
                }
            }
        }
    ...
    if (currentTool === 'pencil' || currentTool === 'eraser') {
       ...
       updateLayerData(activeLayerId, new Uint8ClampedArray(layer.data));
       lastDrawPos.current = { x, y };
    } else if (['line', 'rectangle', 'ellipse'].includes(currentTool) && startDrawPos.current) {
       previewData.fill(0);
       if (currentTool === 'line') drawLine(previewData, startDrawPos.current.x, startDrawPos.current.y, x, y, color, false);
       if (currentTool === 'rectangle') drawRect(previewData, startDrawPos.current.x, startDrawPos.current.y, x, y, color, false);
       if (currentTool === 'ellipse') drawEllipse(previewData, startDrawPos.current.x, startDrawPos.current.y, x, y, color, false);
       lastDrawPos.current = { x, y };
       setPreviewDataVersion(v => v + 1);
    }
  };
  ...
  const handlePointerUp = (e: React.PointerEvent<HTMLDivElement>) => {
    ...
    if (isDrawing) {
        ...
        if (currentTool === 'select' && startDrawPos.current && lastDrawPos.current) {
            ...
            for (let j = Math.max(0, startY); j <= Math.min(dimensions.height - 1, endY); j++) {
                for (let i = Math.max(0, startX); i <= Math.min(dimensions.width - 1, endX); i++) {
                    newSelection[j * dimensions.width + i] = 1;
                }
            }

            if (e.shiftKey && selection) {
                for(let i=0; i<newSelection.length; i++) newSelection[i] = newSelection[i] || selection[i];
            } else if (e.altKey && selection) {
                for(let i=0; i<newSelection.length; i++) newSelection[i] = selection[i] && !newSelection[i] ? 1 : 0;
            }
        ...
        if (currentTool === 'move' && selection && startDrawPos.current && lastDrawPos.current && originalLayerData.current) {
            ...
            if (layer) {
                ...
                for (let i = 0; i < previewData.length; i+=4) {
                    if (previewData[i+3] > 0) { // If preview has pixel
                        const pxIdx = i;
                        // Track new pixel
                        if (!strokeDiff.current.has(pxIdx)) {
                            strokeDiff.current.set(pxIdx, {
                                oldColor: {r: layer.data[pxIdx], g: layer.data[pxIdx+1], b: layer.data[pxIdx+2], a: layer.data[pxIdx+3]},
                                newColor: {r: previewData[pxIdx], g: previewData[pxIdx+1], b: previewData[pxIdx+2], a: previewData[pxIdx+3]}
                            });
                        } else {
                             strokeDiff.current.get(pxIdx)!.newColor = {r: previewData[pxIdx], g: previewData[pxIdx+1], b: previewData[pxIdx+2], a: previewData[pxIdx+3]};
                        }
                        layer.data[pxIdx] = previewData[pxIdx];
                        layer.data[pxIdx+1] = previewData[pxIdx+1];
                        layer.data[pxIdx+2] = previewData[pxIdx+2];
                        layer.data[pxIdx+3] = previewData[pxIdx+3];
                    }
                }
                ...
                updateLayerData(activeLayerId, new Uint8ClampedArray(layer.data));
                commitDrawing(activeLayerId, Array.from(strokeDiff.current.entries()));
    ...
    >
      ...
      >
        <div ref={canvasRef} className="w-full h-full pointer-events-none">
           <Application backgroundAlpha={0} width={dimensions.width} height={dimensions.height} antialias={false} resolution={1}>
              <pixiContainer>
                ...
                <CanvasLayer key={`preview-${previewDataVersion}`} data={previewData} opacity={1} visible={true} dimensions={dimensions} zIndex={layers.length} />
